...
import 'package:mocupsangkar/features/mockup08/data/dummy_db.dart';
import 'package:mocupsangkar/features/mockup08/models/sangkar_models.dart';
...
class _DesainPageState extends State<DesainPage> {
          ...
          Row(
            ...
            children: [
              const Column(
                ...
                children: [
                  Text(
                    'SAKURA MASONRY CATALOG',
                    style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.pinkAccent, letterSpacing: 1.2),
                  ),
                  SizedBox(height: 4),
                  Text('Creative Pinterest-style staggered layout', style: TextStyle(fontSize: 10, color: Colors.pinkAccent)),
                ],
              ),
              ElevatedButton.icon(
                onPressed: () => _tampilkanDialogGenerasiAI(context),
                icon: const Icon(Icons.favorite, size: 14, color: Colors.black87),
                label: const Text('Add Sakura AI', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
            ...
            child: SingleChildScrollView(
              child: Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      children: List.generate((_listDesain.length / 2).ceil(), (index) {
                        final d = _listDesain[index * 2];
                        // Staggered height: alternate card sizes
                        final double imgHeight = index % 2 == 0 ? 140.0 : 200.0;
                        return _buildMasonryCard(d, imgHeight, index * 2);
                      }),
                    ),
                  ),
                  ...
                  Expanded(
                    child: Column(
                      children: List.generate(_listDesain.length ~/ 2, (index) {
                        final d = _listDesain[index * 2 + 1];
                        // Staggered height: opposite size of the left column
                        final double imgHeight = index % 2 == 0 ? 200.0 : 140.0;
                        return _buildMasonryCard(d, imgHeight, index * 2 + 1);
                      }),
                    ),
                  ),
                ],
  ...
  Widget _buildMasonryCard(DesainDecal d, double imgHeight, int idx) {
    return Container(
      margin: const EdgeInsets.only(bottom: 14),
      decoration: BoxDecoration(
        color: const Color(0xFF2C131B),
      ...
      child: Column(
        ...
        children: [
          GestureDetector(
            onTap: () => _tampilkanLightbox(context, d),
            child: ClipRRect(
              borderRadius: const BorderRadius.only(topLeft: Radius.circular(12), topRight: Radius.circular(12)),
              child: Image.network(_getImageUrlForDesain(d), height: imgHeight, fit: BoxFit.cover),
            ),
          ),
          Padding(
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    IconButton(
                      icon: const Icon(Icons.delete, color: Colors.pinkAccent, size: 16),
                      onPressed: () {
                        setState(() {
                ...
                Text('Motif: ${d.motif} · ${d.ukuran}', style: const TextStyle(color: Colors.white54, fontSize: 9)),
              ],
            ),
          )
        ],
          ...
          child: Column(
            ...
            children: [
              AppBar(
                ...
                leading: IconButton(icon: const Icon(Icons.close, color: Colors.pinkAccent), onPressed: () => Get.back()),
              ),
              Expanded(
  ...
  void _tampilkanDialogGenerasiAI(BuildContext context) {
    ...
    Get.dialog(
      AlertDialog(
        backgroundColor: const Color(0xFF1F0E13),
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12), side: const BorderSide(color: Colors.pinkAccent, width: 0.5)),
        title: const Text('Add Sakura Decal', style: TextStyle(color: Colors.pinkAccent, fontSize: 13)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              final newDecal = DesainDecal(
                ...
                nama: nameController.text.isEmpty ? 'Sakura Design' : nameController.text,
                kategori: 'Sakura Style',
                motif: 'Floral',
                ukuran: '30 × 20 cm',
                format: 'PNG',
                resolusi: '300 dpi',
            ...
            style: ElevatedButton.styleFrom(backgroundColor: Colors.pinkAccent, foregroundColor: Colors.black87),
